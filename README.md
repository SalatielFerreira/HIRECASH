...
- **Registro de vagas**: botão próprio (ícone de maleta com "+") ao lado do de adicionar candidato, no mesmo formato — uma telinha para cadastrar, com a lista das vagas já cadastradas logo abaixo, cada uma com editar e excluir. O campo Vaga do cadastro de candidato é essa lista (não texto livre); sem nenhuma vaga cadastrada, o botão de adicionar candidato avisa para cadastrar uma primeiro. **A vaga se encerra sozinha quando um candidato dela é contratado**: o Status da vaga de todos os candidatos que concorrem a ela passa a **Encerrada** (fora das opções escolhíveis, mesmo caso de "Em atividade"/"Baixa" na Etapa), e reabre sozinha (volta a "Publicada") se deixar de haver algum contratado — a atualização aparece na hora nas outras linhas da tabela.
