...
- **5 abas** na barra inferior, nesta ordem: **Candidato** → **Dashboard** (inicial) → **Comissão** → **Relatório** → **Configuração**. A barra flutua descolada da borda da tela, com cantos arredondados.
...
- **Cadastro de candidatos**: botão + na página Candidato abre um modal (fecha pelo X ou por Esc — clicar fora não fecha, pra não perder dados digitados) com o formulário completo; ao salvar, o candidato aparece como uma linha em uma tabela estilo planilha, persistida em `localStorage`. `Ctrl+Z` (`Cmd+Z` no Mac) desfaz o último campo alterado no formulário, tanto texto quanto a escolha de uma opção — um passo por edição completa, não por tecla digitada.
- **Clonar/excluir candidato**: botão direito do mouse em qualquer ponto da linha, na página Candidato, abre um menu de contexto (no modelo do menu nativo do sistema) com "Clonar candidato" e, separado por uma linha, "Excluir candidato" (em vermelho, com confirmação antes de remover). Clonar cria um novo candidato com os mesmos dados, nome com "(cópia)" no final. Dentro de um campo em edição, o menu nativo do navegador continua funcionando normalmente (copiar, colar, corretor).
- **Esc fecha qualquer janela aberta** (modal, coluna de filtro, confirmação) — nunca a página em si.
- **Localização é estado + cidade** (não texto livre): digita o estado (sigla ou nome por extenso) e depois a cidade, entre todos os municípios do Brasil (fonte: IBGE) — o campo sugere numa lista e também autocompleta o resto da palavra conforme se digita (aceita continuar digitando por cima, ou confirma com →/Tab/Enter), sem precisar abrir uma lista fechada. O que fica gravado/exibido é sempre "Cidade - UF".
...
- **Filtro por campo**: botão de funil na frente da busca abre uma coluna lateral que desliza da esquerda, com todos os campos (menos Observação) empilhados — cada um com todos os valores em uso, podendo escolher mais de um por campo (cada escolha vira uma flag removível). Só passa a valer ao clicar em "Filtrar"; "Cancelar" (ou fechar/Esc/clicar fora) descarta sem aplicar. Combina com a busca por texto quando os dois estão ativos. Com um filtro aplicado, um "X" aparece sobre o botão de funil — clicar nele limpa o filtro aplicado na hora, sem abrir a coluna. Dentro da coluna, "Limpar filtro" só esvazia as escolhas feitas ali (as flags somem) — não fecha a coluna nem muda o filtro de verdade, que só muda ao clicar em "Filtrar" ou "Cancelar", como qualquer outra edição feita ali dentro. Um candidato cadastrado enquanto o filtro está ativo aparece normalmente na tabela (não é escondido por um filtro que ele nunca passou por cima) — só passa a valer a regra quando o filtro é aplicado de novo.
...
- **Navegação por setas entre campos editáveis**, como no Excel: com uma célula em edição, ←/→ move para o campo editável ao lado (pulando os que não são editáveis) e ↑/↓ para o mesmo campo do candidato de cima/baixo, pulando linhas escondidas pelo filtro. Em campo de texto, ←/→ só navegam com o cursor na ponta do texto; na Observação (várias linhas), as setas nunca navegam.
- **Arrastar para rolar a tabela**: clicar e arrastar com o mouse em qualquer ponto da tabela de candidatos rola para o lado, sem depender só da barra de rolagem. A rolagem vertical continua só pela barra/roda do mouse.
...
- **Backup dos candidatos e das vagas**: em Configuração, botões de **Exportar** (folha de compartilhamento no celular, "Salvar como" no computador) e **Importar** (explorador de arquivos), gerando um `.json` com os candidatos **e** as vagas cadastradas (código + nome). Como os dados moram no `localStorage` do navegador, esse arquivo é a rede de segurança contra limpar os dados do navegador ou trocar de aparelho. A importação nunca apaga: casa pelo `id` (vagas sem `id` casam pelo código), atualizando quem já existe e acrescentando o resto.
...
│   ├── images/                    # logo-s-mark.png ("S" da marca; "HireCash" e a régua são texto)
...
│       ├── pages/                 # dashboard.js, candidato.js, comissao.js, relatorio.js, configuracao.js
